import { ArrowLeft, CheckCircle2, CreditCard, MessageCircle, ShieldCheck } from "lucide-react";
      ...
      <section className="mx-auto grid w-full max-w-6xl gap-5">
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_0.72fr] lg:items-start">
          ...
          <aside className="grid gap-4 rounded-lg border border-black/10 bg-white p-5 shadow-xl shadow-slate-900/10 lg:sticky lg:top-6">
            <div>
              <p className="text-xs font-black uppercase text-blue-700">Escolha como pagar</p>
              <strong className="mt-1 block text-3xl font-black sm:text-4xl">{money.format(proposal.price)}</strong>
              <p className="mt-2 text-sm font-bold leading-6 text-slate-600">
                Selecione uma opcao. O link sera gerado na hora e a confirmacao atualiza a proposta.
              </p>
            </div>
            ...
            {paid ? (
              ...
              <div className="grid gap-2">
                <PaymentForm
                  amount={money.format(signal30)}
                  brandColor={brandColor}
                  mode="signal_30"
                  slug={proposal.publicSlug}
                  title="Pagar 30% para reservar"
                />
                <PaymentForm
                  amount={money.format(signal50)}
                  brandColor={brandColor}
                  mode="signal_50"
                  slug={proposal.publicSlug}
                  title="Pagar 50% de entrada"
                />
                <PaymentForm
                  amount={money.format(proposal.price)}
                  brandColor={brandColor}
                  mode="full"
                  primary
                  slug={proposal.publicSlug}
